...
ordered_stats = 'avg_nz_female avg_nz_male avg_all avg_female avg_male log2fc log2fc_scanpy padj pct_female pct_male'.split()
...
tmp = adata.layers["bias_scanpy"].todense()
bias_scanpy = tmp.astype(str)
bias_scanpy[tmp > 0] = "Female"
bias_scanpy[tmp < 0] = "Male"
bias_scanpy[tmp == 0] = ""
bias_scanpy = (
    pd.DataFrame(bias_scanpy, index=adata.obs_names, columns=adata.var_names)
...
expr_bias = pd.concat(
    [bias, bias_scanpy] + frames,
    keys = ['bias', 'bias_scanpy'] + ordered_stats,
    names = ['stats', 'cluster'],
    axis = 1
)
print(expr_bias)
...
detailed = (
    ...
    .sort_index(1)
    .reindex(ordered_stats + ['bias', 'bias_scanpy'], axis=1, level='stats')
)
print(detailed)
