...
pdf(
  paste0("GSEA_dotplots_marker_score_", tissue, ".pdf"),
  height = 10, width = 20
)
...
for (cls in clusters) {
  gse <-readRDS(paste0("GO_gsea_", tissue, "/", tissue, "_", cls, ".rds"))
  if (!is.na(gse)) {
  ...
  p <- (
    ...
    + facet_grid(.~.sign)
    + ggtitle(paste("GO GSEA for", tissue, cls))
  ...
  text(x = 0.5, y = 0.5, paste(
    "Either there was some error in running GSEA on this cluster,\n",
    cls,
